...
import '../functions/checkstring.dart';
...
class _SelectedFoodScreenState extends State<SelectedFoodScreen> {
  ...
  void initState() {
    ...
    foodName = food.name.isNotEmpty ? food.name : food.name_tr;
  ...
  void _updateFoodDetails() {
    ...
    foodName = isTurkish ? (food.name_tr.isNotEmpty ? food.name_tr : food.name) : (food.name.isNotEmpty ? food.name : food.name_tr);
    stepsText = _formatInstructions(_instructionsForLocale(isTurkish));
    if (mounted) setState(() {});
  }

  List<String> _instructionsForLocale(bool isTurkish) {
    if (isTurkish && food.instructions_tr.isNotEmpty) return food.instructions_tr;
    if (!isTurkish && food.instructions.isNotEmpty) return food.instructions;
    return isTurkish ? food.instructions_tr : food.instructions;
  }
  ...
  Widget build(BuildContext context) {
    return globalScaffold(
      appBar: globalAppBar(foodName, context),
      body: selectedFoodBody(),
      bottomAppbar: _isAdLoaded ? SizedBox(height: _bannerAd.size.height.toDouble(), child: AdWidget(ad: _bannerAd)) : null,
    );
  }

  Widget selectedFoodBody() {
    final double scrollBottomPad = 100 + (_isAdLoaded ? _bannerAd.size.height.toDouble() : 0);
    final bool hasIngredients = food.ingredients.isNotEmpty || food.ingredientsRaw.isNotEmpty || food.ingredientsRaw_tr.isNotEmpty;
    return Stack(
      children: [
  ...
  Widget _buildRecipeFactsCard() {
    ...
    if (food.cuisines.isNotEmpty) {
      rows.add(_factRow('Cuisine', _stringPills(food.cuisines)));
    }
  ...
  Widget _buildIngredientsContent() {
    final bool isTr = context.locale.languageCode == 'tr';
    final List<String> rawIngredients = isTr ? food.ingredientsRaw_tr : food.ingredientsRaw;
    ...
    final List<IngredientClass> ingredients = food.ingredients;
    return Wrap(
      spacing: 10,
      runSpacing: 10,
      children: List.generate(ingredients.length, (index) {
        final IngredientClass ingredient = ingredients[index];
        final String value = (isTr && ingredient.Ingredient_tr.isNotEmpty) ? ingredient.Ingredient_tr : ingredient.Ingredient;
        return Container(
          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
          decoration: BoxDecoration(color: AppTheme.secondaryColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(18)),
  ...
  Future<void> _copyUrlToClipboard(String urlText) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Link panoya kopyalandı: $urlText')),
    );
  }
  ...
  void _shareRecipe() async {
    ...
    final String text = 'Bu tarifi denemelisin! 👉 $recipeUrl';
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Paylaşım açılamadı, link panoya kopyalandı.')),
      );
    }
  }
