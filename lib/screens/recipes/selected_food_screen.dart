...
class _SelectedFoodScreenState extends State<SelectedFoodScreen> {
  ...
  void initState() {
    ...
    if (FavoritesStore.isFavorite(food.recipeId) &&
        _hasFullRecipeDetails(food)) {
      FavoritesStore.cacheFood(food, category: _primaryCategoryLabel);
    }
    ...
    if (widget.recipeId != 0 &&
      ...
      _fetchSingleFood(widget.recipeId)
          .then((fetchedFood) {
            if (!mounted) return;
            if (fetchedFood != null) {
              setState(() {
              ...
              if (FavoritesStore.isFavorite(fetchedFood.recipeId)) {
                FavoritesStore.cacheFood(
                  fetchedFood,
          ...
          .catchError((e) {
            AppLogger.w('Error fetching food', e);
          });
    }
  }
    ...
    _bannerAd = BannerAd(
      ...
      listener: BannerAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          ad.dispose();
          AppLogger.w('Banner ad load failed', error);
        },
      ),
  ...
  bool _hasFullRecipeDetails(Food currentFood) {
    return currentFood.instructions.isNotEmpty ||
        currentFood.instructionsTr.isNotEmpty ||
        currentFood.ingredients.isNotEmpty ||
        currentFood.ingredientsRaw.isNotEmpty ||
        currentFood.ingredientsRawTr.isNotEmpty;
  }
  ...
  void _updateFoodDetails() {
    ...
    foodName = isTurkish
        ? (food.nameTr.isNotEmpty ? food.nameTr : food.name)
        : (food.name.isNotEmpty ? food.name : food.nameTr);
  ...
  String _formatInstructions(List<String> instructions) {
    ...
    return List.generate(
      instructions.length,
      (index) => '${index + 1}. ${instructions[index]}',
    ).join('\n\n');
  }
  ...
  Widget _buildMetaHighlights() {
    ...
    final servingsText = servingsValue != null
  ...
  Widget _buildRecipeFactsCard() {
    ...
    final String recipeInfoLabel = recipeInfoTitle == 'recipeInfo'
        ? (isTr ? 'Tarif Bilgileri' : 'Recipe Info')
        : recipeInfoTitle;
    ...
    final Widget? ratingWidget = _ratingBadge(
      food.ratingValue,
      food.ratingCount,
    );
  ...
  Widget? _ratingBadge(double? ratingValue, int? ratingCount) {
    ...
    final String countText = ratingCount != null
        ? (isTr ? '($ratingCount oy)' : '($ratingCount ratings)')
        : (isTr ? '(oy yok)' : '(no ratings)');
  ...
  Widget _buildIngredientsContent() {
    final bool isTr = context.locale.languageCode == 'tr';
    final List<String> rawIngredients = isTr
  ...
  Widget _buildTimeInfoContent() {
    final List<_InfoItem> infoItems = [
      _InfoItem(
        icon: Icons.timer,
        label: 'prepTime'.tr(),
        value: formatDuration(food.prepTimeMinutes),
      ),
      _InfoItem(
        icon: Icons.local_fire_department,
        label: 'cookTime'.tr(),
        value: formatDuration(food.cookTimeMinutes),
      ),
      _InfoItem(
        icon: Icons.timelapse,
        label: 'totalTime'.tr(),
        value: formatDuration(food.totalTimeMinutes),
      ),
  ...
  Future<void> _copyUrlToClipboard(String urlText) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('linkCopied'.tr(args: [urlText]))));
  }
  ...
  void _shareRecipe() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('shareFallback'.tr())));
    }
  }
