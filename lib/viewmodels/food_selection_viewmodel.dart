...
class FoodSelectionViewModel extends ChangeNotifier {
  ...
  List<IngredientClass> get selectedIngredients => List.unmodifiable(_formDTO.ingredientList);
  ...
  void init({required List<IngredientClass> initialIngredients}) {
    _formDTO.ingredientList = List.from(initialIngredients);
  }

  void updateSelectedIngredients(List<IngredientClass> items) {
    _formDTO.ingredientList = List.from(items);
    notifyListeners();
  }
  ...
  Future<String?> startFiltering() async {
    ...
    try {
      final List<String> selectedIngredients = _formDTO.ingredientList.map((e) => e.Ingredient).toList();
